...
for (const [name, source] of Object.entries(OFFLINE_STYLE.sources)) {
  if (source.type !== 'raster' || !('tiles' in source) || !source.tiles?.[0]) continue;
  // Spot-check the z0 tile — the full pyramid is generated by `npm run gen:relief`.
  const rel = source.tiles[0].replace(/^\/+/, '').replace('{z}/{x}/{y}', '0/0/0');
  const file = path.join(publicDir, rel);
  if (!fs.existsSync(file)) {
    errors.push(`basemap raster source "${name}": missing ${rel} — run \`npm run gen:relief\``);
  }
}
